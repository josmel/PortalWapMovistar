...
//                }
   ...
   /** function ordenarBanners($BannerData) { 
        ...
        foreach (array(1, 2, 3, 4, 5) as $i) {
            if (count($BannerData[0][$i]) == 2) {
                $pathTwoBanner = $config->app->$i->jsonTwoBanner;
                $TwoBanner = json_decode(file_get_contents($pathTwoBanner), true);
                switch ($TwoBanner) {
                    case 1:
                ...
                $fp = fopen($pathTwoBanner, "w")
                        or die("Error al abrir fichero de salida");
                fwrite($fp, json_encode($contador, 128));  //JSON_PRETTY_PRINT=128
                fclose($fp);
            } else {
                $ordenNUm[$i] = array('0' => $BannerData[0][$i][0]);
            }
     ...
     function ordenarBanners($BannerData) {
        ...
        foreach (array(1, 2, 3, 4, 5) as $i) {
            if (count($BannerData[0][$i]) >= 3) {
                $pathThreeBanner = $config->app->$i->jsonThreeBanner;
                $ThreeBanner = json_decode(file_get_contents($pathThreeBanner), true);
                switch ($ThreeBanner) {
                    case 1:
                        $ordenNUm[$i] = array('0' => $BannerData[0][$i][0],
                            '1' => $BannerData[0][$i][1],
